...
class StorageManager
{
    ...
    public function upload(Request $request)
    {
        ...
        $path = $this->store($file, $filename);
        if(config('ueditor.disk') == 'oss')
        {
            $response = [
                'state' => 'SUCCESS',
                'url' => env('OSS_IMG_ENDPOINT').$path,
                'title' => $path,
    ...
    protected function store(UploadedFile $file, $filename)
    {
        if(config('ueditor.disk') == 'oss')
        {
            return Storage::disk('oss')->putFile($file->getRealPath(),$file);
        }
